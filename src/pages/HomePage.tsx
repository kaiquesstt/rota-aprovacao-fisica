  ...
  return <div className="home-grid">
    <section className="home-main">
      ...
      <div className="dashboard-grid two">
        ...
        <article className="panel chart-panel"><div className="panel-title"><div><span className="eyebrow">CONTEÚDOS</span><h3>Distribuição dos status</h3></div></div><div className="donut-wrap"><ResponsiveContainer width="55%" height={240}><PieChart><Pie data={statusData} dataKey="value" innerRadius={58} outerRadius={88} paddingAngle={2}>{statusData.map(x=><Cell key={x.name} fill={x.color}/>)}</Pie><Tooltip/></PieChart></ResponsiveContainer><div className="legend-list">{statusData.map(x=><div key={x.name}><i style={{background:x.color}}></i><span>{x.name}</span><b>{x.value}</b></div>)}</div></div></article>
      </div>
