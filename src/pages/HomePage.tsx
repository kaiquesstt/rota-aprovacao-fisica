  ...
  return <div className="home-grid">
    <section className="home-main">
      ...
      <div className="dashboard-grid two">
        <article className="panel chart-panel"><div className="panel-title"><div><span className="eyebrow">EVOLUÇÃO</span><h3>Últimas 8 semanas</h3></div><button className="text-btn" onClick={()=>go('performance')}>Ver detalhes →</button></div><ResponsiveContainer width="100%" height={260}><ComposedChart data={series}><CartesianGrid strokeDasharray="3 3" stroke="#e6edf3"/><XAxis dataKey="week"/><YAxis/><Tooltip/><Bar dataKey="questions" fill="#7db8f5" opacity={.45}/><Line type="monotone" dataKey="accuracy" stroke="#15a6b6" strokeWidth={3}/></ComposedChart></ResponsiveContainer></article>
        <article className="panel chart-panel"><div className="panel-title"><div><span className="eyebrow">CONTEÚDOS</span><h3>Distribuição dos status</h3></div></div><div className="donut-wrap"><div className="donut-chart-shell"><ResponsiveContainer width="100%" height={250}><PieChart><Pie data={statusData} dataKey="value" innerRadius={58} outerRadius={88} paddingAngle={2}>{statusData.map(x=><Cell key={x.name} fill={x.color}/>)}</Pie><Tooltip/></PieChart></ResponsiveContainer></div><div className="legend-list">{statusData.map(x=><div key={x.name}><i style={{background:x.color}}></i><span>{x.name}</span><b>{x.value}</b></div>)}</div></div></article>
      </div>
      <div className="dashboard-grid three">
        <article className="panel"><span className="eyebrow">DESEMPENHO POR ÁREA</span><h3>Leitura rápida</h3><ResponsiveContainer width="100%" height={210}><BarChart data={disc.slice(0,6)}><CartesianGrid strokeDasharray="3 3" stroke="#e8eef3"/><XAxis dataKey="name" tick={{fontSize:10}}/><YAxis domain={[0,100]}/><Tooltip/><Bar dataKey="score" fill="#238ed3" radius={[6,6,0,0]}/></BarChart></ResponsiveContainer></article>
