...
type Store={
  ...
  startTimer:(topicId:string,preset?:number)=>void; toggleTimer:()=>void; resetTimer:()=>void; tick:()=>void; finishTimer:()=>void; setTimerTopic:(topicId:string)=>void; setTimerPreset:(preset:number,topicId?:string)=>void;
...
export const useAppStore=create<Store>((set,get)=>({
  data:defaultState(),hydrated:false,migrationSource:'',timer:{topicId:null,seconds:25*60,running:false,startedAt:null,preset:25,type:'Teoria'},
  ...
  deleteQuestionSession:(id)=>set(st=>{const data={...st.data,questionSessions:st.data.questionSessions.filter(q=>q.id!==id),errors:st.data.errors.filter(e=>e.questionSessionId!==id)};scheduleSave(data);return {data}}),
  ...
  setTimerPreset:(preset,topicId)=>set(st=>({timer:{...st.timer,topicId:topicId||st.timer.topicId,seconds:preset*60,preset,running:false,startedAt:null}})),
  tick:()=>set(st=>st.timer.running&&st.timer.seconds>0?({timer:{...st.timer,seconds:st.timer.seconds-1}}):({timer:{...st.timer,running:false}})),
  finishTimer:()=>set(st=>{if(!st.timer.topicId)return {timer:{...st.timer,running:false,seconds:st.timer.preset*60}};const t=topics.find(x=>x.id===st.timer.topicId);const elapsed=(st.timer.preset*60)-st.timer.seconds;if(elapsed<=0)return {timer:{...st.timer,running:false,seconds:st.timer.preset*60,startedAt:null}};const session={id:uid(),topicId:t?.id,topicTitle:t?.title||'Sessão',discipline:t?.discipline,group:t?.group,type:st.timer.type,durationSeconds:Math.max(1,elapsed),date:today(),createdAt:new Date().toISOString()};const data={...st.data,sessions:[session,...st.data.sessions]};scheduleSave(data);return {data,timer:{...st.timer,running:false,seconds:st.timer.preset*60,startedAt:null}}}),
